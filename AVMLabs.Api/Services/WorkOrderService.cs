...
namespace AVMLabs.Api.Services
{
    public class WorkOrderService : IWorkOrderService
    {
        ...
        public async Task<WorkOrderResponseDto> CreateWorkOrderAsync(CreateWorkOrderDto dto)
        {
            ...
            var hasOstTest = tests.Any(x => x.IsOST);
            ...
            if (hasOstTest)
            {
                ...
                if (!nbl.IsNbl)
                    throw new BusinessException("OST tests can only be added for NBL clients.");
            }
        ...
        public async Task<WorkOrderResponseDto> UpdateStatusAsync(int woId, UpdateWorkOrderStatusDto dto)
        {
            ...
            if (dto.Status == Constants.WorkOrderStatus.Billed)
            {
                var existingInvoice = await _context.Invoices
                    .AnyAsync(x => x.WOId == workOrder.WOId);
        ...
        public async Task<List<WorkOrderResponseDto>> GetWorkOrdersAsync()
        {
            var workOrders = await _context.WorkOrders
                ...
                .Include(x => x.WorkOrderItems)
                .ThenInclude(x => x.Test)
                .OrderByDescending(x => x.WODate)
                .ThenByDescending(x => x.WOId)
                .ToListAsync();

            return workOrders.Select(x => MapToResponse(x, x.Client)).ToList();
        }
